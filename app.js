...
const SESSION_KEY = "chesakl-toolflow-active-session-v1";
...
function restoreSession() {
  try {
    ...
    const worker = db.workers.find(w => w.id === saved.workerId);
    if (!worker) {
      localStorage.removeItem(SESSION_KEY);
      return;
    }
    selectedWorker = worker;
    sessionItems = Array.isArray(saved.items) ? saved.items : [];
    ...
    renderSession();
    setStatus("", `Список открыт: ${worker.name}`, "Касайтесь меток инструментов. Повторная метка сотрудника закроет список", "2");
  } catch {
    localStorage.removeItem(SESSION_KEY);
  }
}
...
async function copyTagLink(tag, button) {
...
function renderSettings() {
  $("#workerList").innerHTML = db.workers.map(w => `<div class="item">
    <div><div class="item-title">${escapeHtml(w.name)}</div><div class="item-sub">${escapeHtml(w.tag)}</div></div>
    <div class="item-actions">
      <button class="mini copy-link" data-copy-tag="${escapeHtml(w.tag)}">Скопировать ссылку</button>
      <button class="mini" data-del-worker="${w.id}">Удалить</button>
    </div>
  </div>`).join("");

  $("#settingsToolList").innerHTML = db.tools.map(t => `<div class="item">
    <div><div class="item-title">${escapeHtml(t.name)}</div><div class="item-sub">${escapeHtml(t.tag)} · ${t.status}</div></div>
    <div class="item-actions">
      <button class="mini copy-link" data-copy-tag="${escapeHtml(t.tag)}">Скопировать ссылку</button>
      <button class="mini" data-toggle-tool="${t.id}">${t.status==="broken"?"Исправен":"Неисправен"}</button>
      <button class="mini" data-del-tool="${t.id}">Удалить</button>
    </div>
  </div>`).join("");
}
...
$("#workerList").addEventListener("click", e => {
  const copy=e.target.dataset.copyTag;
  if(copy) return copyTagLink(copy, e.target);
...
const incomingTag = new URLSearchParams(location.search).get("tag");
if (incomingTag) {
  history.replaceState({}, "", location.pathname);
  setTimeout(() => processTag(incomingTag), 120);
}
